    ...
    describe "GoodifyInfoPlistAction" do
      ...
      context "WHEN providing a missing or invalid plist" do
        it "THEN it fails appropriately when no plist file provided" do
          goodify_info_plist_no_plist = "lane :test do
            goodify_info_plist ({
              good_entitlement_id: 'com.example.app.good'
            })
          end"
        ...
        it "THEN it fails appropriately when an invalid plist file provided" do
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_invalid_plist).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/Non-existant plist file for GoodifyInfoPlistAction given/)
            end
          )
        end
      end
      ...
      context "WHEN providing a missing or invalid Good App Id" do
        ...
        it "THEN it fails appropriately when no Good App Id provided" do
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_no_good_id).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/No Good ID for GoodifyInfoPlistAction given/)
            end
          )
        end
      ...
      context "WHEN invalid Good Version Numbers" do
        ...
        it "THEN it fails appropriately when the Good Version Number begins with 0" do
          goodify_info_plist_version_number_starts_with_zero = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_version_number_starts_with_zero).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/Invalid Good app version for GoodifyInfoPlistAction given/)
            end
          )
        end

        it "THEN it fails appropriately when the Good Version Number's first section has a non-digit" do
          goodify_info_plist_version_number_starts_with_nondigit = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_version_number_starts_with_nondigit).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/Invalid Good app version for GoodifyInfoPlistAction given/)
            end
          )
        ...
        it "THEN it fails appropriately when the Good Version Number's next section starts with 0" do
          goodify_info_plist_version_number_section_starts_with_zero = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_version_number_section_starts_with_zero).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/Invalid Good app version for GoodifyInfoPlistAction given/)
            end
          )
        ...
        it "THEN it fails appropriately when the Good Version Number's next section contains a non-digit" do
          goodify_info_plist_version_number_section_contains_non_digit = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_version_number_section_contains_non_digit).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/Invalid Good app version for GoodifyInfoPlistAction given/)
            end
          )
        ...
        it "THEN it fails appropriately when the Good Version Number has a section with too many numbers" do
          goodify_info_plist_version_number_section_too_many_numbers = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_version_number_section_too_many_numbers).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/Invalid Good app version for GoodifyInfoPlistAction given/)
            end
          )
        ...
        it "THEN it fails appropriately when the Good Version Number has too many sections" do
          goodify_info_plist_version_number_section_too_many_sections = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_version_number_section_too_many_sections).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
              expect(error.message).to match(/Invalid Good app version for GoodifyInfoPlistAction given/)
            end
          )
      ...
      context "WHEN invalid export method is given" do
        ...
        it "THEN it fails appropriately" do
          goodify_info_plist_invalid_export_method = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_invalid_export_method).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
      ...
      context "WHEN invalid build_simulation_mode is given" do
        ...
        it "THEN it fails appropriately" do
          goodify_info_plist_invalid_build_simulation_mode = "lane :test do
            goodify_info_plist ({
              plist: '#{@temp_plistfile.path}',
              good_entitlement_id: 'com.example.app.good',
              good_entitlement_version: '92.12.12.42',
          ...
          expect { Fastlane::FastFile.new.parse(goodify_info_plist_invalid_build_simulation_mode).runner.execute(:test) }.to(
            raise_error(FastlaneCore::Interface::FastlaneError) do |error|
